...
class loss(keras.layers.Layer):
    ...
    def call(self,inputs):
        ...
        x = torch.FloatTensor(self.n_pts,1).uniform_(0, np.pi)
        #x,_ = x_r.sort()
        x.requires_grad = True
        
        u = self.u_model(x) 

        dux = torch.autograd.grad(u,x,grad_outputs=torch.ones_like(u),
            create_graph=True)[0]
        
        duxx = torch.autograd.grad(dux,x,grad_outputs=torch.ones_like(dux),
            create_graph=True)[0]
...
plt.plot(xlist, u_model(xlist).detach().numpy(), color='b')
plt.plot(xlist, exact_u(xlist).detach().numpy(), color='m')
