...
class loss(keras.layers.Layer):
    ...
    def call(self, inputs):
        """
        Computes the Deep Fourier Residual (DFR) loss.
        ...
        # Gradients are calculated with vector-jacobian products (JAX PART)
        y, vjp_fn = jax.vjp(self.u_model, self.pts)
        du = vjp_fn(keras.ops.ones(y.shape))[0]
...
# Big model including the Deep Fourier Residual (DFR) loss
...
xlist = jnp.array([np.pi/1000 * i for i in range(1000)])
